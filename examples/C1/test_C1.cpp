...
void test_C1(ExaDiSApp* exadis)
{
    ...
    ctrl.printfreq = 10;
    ctrl.outfreq = 10;
    std::string outputdir = "output_test";
    
    // Initialization
    Vec3 burg = 0.5*Vec3(1.0, 1.0, 1.0);
    Vec3 plane = Vec3(-1.0, 1.0, 0.0).normalized();

    SerialDisNet* network = new SerialDisNet(Lbox);
    network->cell.xpbc = network->cell.ypbc = network->cell.zpbc = PBC_BOUND;
    Vec3 origin = network->cell.center();

    insert_infinite_line(network, burg, plane, 90.0, origin, Mat33().eye(), maxseg);

    SerialDisNet* config = network;
